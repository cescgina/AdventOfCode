...
typedef std::tuple<int, nodes, int, int> info;
...
int main(int argc, char** argv){
    ...
    size_t strength = 0, max_strength = 0;
    int x = 0, y = 0;
    ...
    nodes visited;
    paths.push(std::make_tuple(strength, visited, x, y));
    info element;
    while (not paths.empty()){
        element = paths.top();
        paths.pop();
        strength = std::get<0>(element);
        ...
        for (auto it=all_nodes.begin(); it != all_nodes.end(); it++){
            ...
            if (y == it->first or y == it->second){
                ...
                paths.push(std::make_tuple(strength+it->first+it->second, new_visited, new_x, new_y));
            }
        }
        if (!can_extend){
