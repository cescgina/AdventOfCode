...
int calculate_scores(properties& food, contents& ingredients, names& aunts){
    ...
    for (auto& name: aunts){
        ...
        for (auto it=ingredients.begin(); it != ingredients.end(); it++){
            if (aunt_list.find(it->first) != aunt_list.end() and aunt_list[it->first] != it->second){
               // if a property is found in the aunt's list but not in the
               // exact quantity discard the aunt
               is_aunt = false; 
               break;
            }
        }
