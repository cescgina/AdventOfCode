...
int main(int argc, char** argv){
    ...
    int cost = 0, optimal_cost = 0, turns_player = 0, turns_boss = 0;
        ...
        for (auto a_it=armors.begin(); a_it != armors.end(); a_it++){
            for (auto r_it=rings.begin(); r_it != rings.end(); r_it++){
                for (auto r_it_2=rings.begin(); r_it_2 != rings.end(); r_it_2++){
                    ...
                    // can't win
                    if (dmg <= boss_armor) {
                        optimal_cost = std::max(cost, optimal_cost);
                        continue;
                    }
                    if (boss_dmg <= armor){
                        continue;
    ...
    std::cout << "Most expensive combination costs " << optimal_cost << std::endl;
}
