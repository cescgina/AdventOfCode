...
int main(int argc, char** argv){
    ...
    coords current = std::make_pair(1, 1);
    coords end_state = std::make_pair(final_x, final_y);
    ...
    std::cout << "Reachable states are " << std::count_if(distances.begin(), distances.end(), [](std::pair<coords, int> x) {return x.second <= 50; }) << std::endl;
}
