...
std::string decipher(std::string let, int id){
    int ascii = static_cast<int>(let[0])-97;
    ascii = (ascii + (id % 26)) % 26;
    let = (char) ascii+97;
    return let;
}
...
int main(int argc, char** argv){
    ...
    int checksum, last_freq = 0, index = 0;
    for (std::string line; std::getline(input_file, line);){
        ...
        for (size_t i=0; i<parsed_line.size()-1; i++){
            for (size_t j=0; j<parsed_line[i].size(); j++){
                letter = parsed_line[i][j];
                std::cout << decipher(letter, checksum);
            }
            std::cout  << " ";
        }
        std::cout << std::endl;
