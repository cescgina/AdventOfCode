...
bool is_idle(std::vector<Computer> network){
    for (auto& computer: network){
        if (not computer.is_idle()) return false;
    }
    return true;
}
...
int main(int argc, char** argv){
    ...
    long long nat_x = 0, nat_y = 0;
    std::deque<long long> nat_memory;
    while (true){
        for (auto& computer: network){
            if (DEBUG) std::cout << "Running computer " << computer.index << std::endl;
            ...
            if (DEBUG) std::cout << "Procesing packet " << address << " " << x <<","<<y<< std::endl;
            if (address == 255){
                nat_x = x;
                nat_y = y;
                continue;
        ...
        if (is_idle(network)){
            if (DEBUG) std::cout << "Nat sending " << nat_x << "," << nat_y << std::endl;
            network[0].send_value(nat_x);
            network[0].send_value(nat_y);
            if (not nat_memory.empty() and nat_y == nat_memory.back()){
                std::cout << nat_y << std::endl;
                return 0;
            }
            nat_memory.push_back(nat_y);
        }
    }
}
