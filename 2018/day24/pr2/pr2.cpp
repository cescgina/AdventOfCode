...
typedef std::vector<Group> units_vector;
...
int attack_enemy(const std::shared_ptr<Group>& ptr, matching& attack_matching){
    ...
    if (DEBUG){
        if (ptr->type == 'I'){
            std::cout << "Immune System group " << ptr->id << " attacks defending group " << enemy->id << ", killing " << units_killed << " units" <<std::endl;
        }
        else{
            std::cout << "Infection group " << ptr->id << " attacks defending group " << enemy->id << ", killing " << units_killed << " units" <<std::endl;
        }
    }
    return units_killed;
}
...
void find_move(const std::shared_ptr<Group>& ptr, units& all_groups, matching& attack_matching, units& selected_defenders){
    ...
    for (const auto& el: all_groups){
        ...
        if (DEBUG){
            if (ptr->type == 'I'){
                std::cout << "Immune System group " << ptr->id << " would deal defending group " << el->id << " " << dmg << " damage" <<std::endl;
            }
            else{
                std::cout << "Infection group " << ptr->id << " would deal defending group " << el->id << " " << dmg << " damage" <<std::endl;
            }
        }
    }
    if (enemy){
...
int main(int argc, char** argv){
    ...
    input_file.close();
    int boost_quantity = 1;
    while (true){
        units unit_set;
        units unit_set_cp;
        units_initiative units_sorted_ini;
        units_vector units_cp = unit_set_orig;
        for (auto el: units_cp){
            if (el.type == 'I'){
                el.attack += boost_quantity;
                el.effective = el.attack*el.units;
            }
            unit_set.insert(std::make_shared<Group>(el));
        }
        ...
        while (true){
            ...
            for (const auto& el: units_sorted_ini){
                ...
                total_units_killed += attack_enemy(el, group_to_attack);
            }
            if (total_units_killed == 0){
                // if not units are kill, we are stuck in an infinite battle
                // with immune units
                break;
            }
            // clean dead units
            ...
            if (finish_combat(unit_set)){
                ...
                if ((*unit_set.begin())->type == 'I'){
                    std::cout << "Finished combats with " << remaining_units << " units and boost " << boost_quantity << std::endl;
                    return 0;
                }
                else{
                    break;
