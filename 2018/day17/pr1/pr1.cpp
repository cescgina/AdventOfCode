...
bool is_walled(Ray& ray, map_clay& current_state){
    bool left_wall = is_clay(ray.x_left, ray.y, current_state); 
    bool right_wall = is_clay(ray.x_right, ray.y, current_state);
    int new_left = ray.x_left+1, new_right = ray.x_right-1;
    while (new_left < ray.x){
        if (is_clay(new_left, ray.y, current_state)){
            ray.x_left = new_left;
            left_wall = true;
        }
        new_left++;
    }
    while (new_right > ray.x){
        if (is_clay(new_right, ray.y, current_state)){
            ray.x_right = new_right;
            right_wall = true;
        }
        new_right--;
    }
...
void determine_overflow_coords(Ray& ray, map_clay& current_state, rays_set& to_add, int& id){
    if (is_clay(ray.x_left, ray.y, current_state)){
        for(int l=ray.x_left+1; l<=ray.x_right+1; l++){
            // add filled spaces to the state
            current_state[std::make_pair(ray.y, l)] = 2;
        }
        ray.x = ray.x_right+1;
    }
    else if (is_clay(ray.x_right, ray.y, current_state)){
        ray.x = ray.x_left-1;
        for(int l=ray.x_right-1; l>=ray.x_left-1; l--){
            // add filled spaces to the state
            current_state[std::make_pair(ray.y, l)] = 2;
        }
    }
        ...
        ray.x = ray.x_right+1;
        id++;
        to_add.push_back(Ray {ray.x_left-1, ray.y, id, 0, 0});
    }
}
...
int main(int argc, char** argv){
    ...
    while (not rays.empty()){
        ...
        for (auto it = rays.begin(); it != rays.end(); ){
            //if (visited_sources.find(std::make_pair(it->x, it->y)) != visited_sources.end()){
            //    it++;
            //    continue;
            //}
            //visited_sources.insert(std::make_pair(it->x, it->y));
            std::cout << "Studying ray " << it->id << " at pos " << it->x << "," << it->y << " with walls between " << it->x_left << " and " << it->x_right << std::endl;
            ...
            if (it->y >= y_max) {
                ...
                // these cases should be accounted for by the existing functions
                //else if (kind_wall == 1){
                ...
                //}
                    ...
                    std::cout << "Bottom of the pit at " <<it->x << "," << it->y << std::endl;
                    find_walls(*it, state);
                    it++;
                }
            }
        ...
        for (auto it = rays.begin(); it != rays.end(); it++){
            ...
            std::cout << "Finished well " << it->id << " at pos " << it->x << "," << it->y << std::endl;
        }
    ...
    print_state(x_min-1, x_max+1, y_min-1, y_max+1, state);
    std::cout << count_tiles(state, y_min, y_max) << " wet tiles" << std::endl;
}
